...
/*   Updated: 2022/04/03 12:42:46 by pvznuzda         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*check_buffer_nl(char buffer[], char *result, char **rem, int nl_n_i[])
{
	...
	while (buffer[++i] != '\0')
	{
		if (buffer[i] == '\n')
		{
			nl_n_i[0] = 1;
			buffer[i] = '\0';
			temp = ft_strdup(buffer, 1);
			res = ft_strjoin(result, temp);
			free(result);
			free(temp);
			temp = ft_strdup(buffer + i + 1, 0);
			free(*rem);
			*rem = temp;
			return (res);
		}
...
char	*check_remainder_nl(char *result, char **rem, int nl_n_i[])
{
	...
	while ((*rem)[++nl_n_i[1]] != '\0')
	{
		if ((*rem)[nl_n_i[1]] == '\n')
		{
			...
			free(result);
			free(temp);
			temp = ft_strdup((*rem) + nl_n_i[1] + 1, 0);
			free(*rem);
			*rem = temp;
			return (res);
		}
	}
	res = ft_strjoin(result, *rem);
	free(result);
	free(*rem);
	*rem = NULL;
	return (res);
